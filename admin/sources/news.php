	...
	function save_item()
	{
		...
		if($id && $act!='save_copy')
		{
			...
			if(isset($_FILES['file-taptin']))
			{
				...
				if($file_taptin = $func->uploadImage("file-taptin", $config['news'][$type]['file_type'], UPLOAD_FILE,$file_name))
				{
					$data['file_taptin'] = $file_taptin;
					$row = $d->rawQueryOne("select id, file-taptin from #_news where id = ? and type = ? limit 0,1",array($id,$type));
					if($row['id']) $func->delete_file(UPLOAD_FILE.$row['file_taptin']);
				}
			}
