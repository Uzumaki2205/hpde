            ...
            <ul class="nav nav-pills nav-sidebar flex-column nav-child-indent text-sm" data-widget="treeview" role="menu" data-accordion="false">
                ...
                <?php if(isset($config['title'])) { ?>
                    <?php
                        ...
                        $active = "";
                        $menuopen = "";
                        if(isset($kiemtra) && $kiemtra == true) if($func->check_access('title', 'photo_static', '', 'phrase-2') && $func->check_access('title', 'man_photo', '', $config['title']['man_photo'], 'phrase-2')) $none = "d-none";
                        if($com=='title' && !isset($disabled['title'][$_GET['type']]))
                        {
                            $active = 'active';
                            $menuopen = 'menu-open';
                        }
                    ?>
                    <li class="nav-item has-treeview <?=$menuopen?> <?=$none?>">
                        <a class="nav-link <?=$active?>" href="#" title="Quản lý tiêu đề">
                            <i class="nav-icon text-sm fas fa-h-square"></i>
                            <p>
                                Quản lý tiêu đề
                                <i class="right fas fa-angle-left"></i>
                            </p>
                        ...
                        <ul class="nav nav-treeview">
                            <?php if(isset($config['title']['man_photo'])) { ?>
                                <?php foreach($config['title']['man_photo'] as $k => $v) { if(!isset($disabled['title'][$k])) {
                                    $none = "";
                                    $active = "";
                                    if(isset($kiemtra) && $kiemtra == true) if($func->check_access('photo', 'man_photo', $k, null, 'phrase-1')) $none = "d-none";
                                    if($com=='title' && $_GET['type']==$k && ($act=='man_photo' || $act=='add_photo' || $act=='edit_photo')) $active = "active"; ?>
                                    <li class="nav-item <?=$none?>">
                                        <a class="nav-link <?=$active?>" href="index.php?com=title&act=man_photo&type=<?=$k?>" title="<?=$v['title_main_photo']?>"><i class="nav-icon text-sm far fa-caret-square-right"></i><p><?=$v['title_main_photo']?></p></a>
                                    </li>
                                <?php } } ?>
                            <?php } ?>
                        </ul>
