<?php
    ...
    $bonus = (isset($item['bonus']) && $item['bonus'] != '') ? json_decode($item['bonus'],true) : null;
...
<section class="content">
    <form class="validation-form" novalidate method="post" action="<?=$linkSave?>" enctype="multipart/form-data">
                            ...
                            <div class="card-body card-article">
                                <div class="tab-content" id="custom-tabs-three-tabContent-lang">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <strong class="d-block mt-2 mb-2 text-sm"> <?= $config['news'][$type]['file_type']; ?></strong>
                                        ...
                                        <?php if(isset($item['file_taptin']) && $item['file_taptin'] != '') { ?>
                                        <a class="btn btn-sm bg-gradient-primary text-white d-inline-block align-middle p-2 rounded mb-1" href="<?=UPLOAD_FILE.@$item['file_taptin']?>" title="Download tập tin hiện tại"><i class="fas fa-download mr-2"></i>Download tập tin hiện tại</a>
                                        <?php } ?>
                                    </div>
                                    <?php } ?>
                                    <?php foreach($config['website']['lang'] as $k => $v) { ?>
                                    ...
                                    <?php } ?>
                                    <?php if ($config['news'][$type]['bonus']){ ?>
                                    <?php if ($config['news'][$type]['bonus_arr']['address']) { ?>
                                    <div class="form-group">
                                        <label for="bonus-address"> <?=$config['news'][$type]['bonus_arr']['address_text']?></label>
                                        <input type="text" class="form-control for-seo" name="data[bonus][address]" id="bonus-address" placeholder="<?=$config['news'][$type]['bonus_arr']['address_text']?>" value="<?=(isset($bonus['address']) && $bonus['address'] != '') ? $bonus['address'] : ''?>">
                                    </div>
                                    <?php } ?>
                                    <?php if ($config['news'][$type]['bonus_arr']['hotline']) { ?>
                                    <div class="form-group"> 
                                        <label for="bonus-hotline"> <?=$config['news'][$type]['bonus_arr']['hotline_text']?></label>
                                        <input type="text" class="form-control for-seo" name="data[bonus][hotline]" id="bonus-hotline" placeholder="<?=$config['news'][$type]['bonus_arr']['hotline_text']?>" value="<?=(isset($bonus['hotline']) && $bonus['hotline'] != '') ? $bonus['hotline'] : ''?>">
                                    </div>
                                    <?php } ?>
                                    <?php if ($config['news'][$type]['bonus_arr']['email']) { ?>
                                    <div class="form-group">
                                        <label for="bonus-email"> <?=$config['news'][$type]['bonus_arr']['email_text']?></label>
                                        <input type="text" class="form-control for-seo" name="data[bonus][email]" id="bonus-email" placeholder="<?=$config['news'][$type]['bonus_arr']['email_text']?>" value="<?=(isset($bonus['email']) && $bonus['email'] != '') ? $bonus['email'] : ''?>">
                                    </div>
                                    <?php } ?>
                                    <?php if ($config['news'][$type]['bonus_arr']['website']) { ?>
                                    <div class="form-group">
                                        <label for="bonus-website"> <?=$config['news'][$type]['bonus_arr']['website_text']?></label>
                                        <input type="text" class="form-control for-seo" name="data[bonus][website]" id="bonus-website" placeholder="<?=$config['news'][$type]['bonus_arr']['website_text']?>" value="<?=(isset($bonus['website']) && $bonus['website'] != '') ? $bonus['website'] : ''?>">
                                    </div>
                                    <?php } ?>
                                    <?php if ($config['news'][$type]['bonus_arr']['iframe']) { ?>
                                    <div class="form-group">
                                        <label for="bonus-iframe"> <?=$config['news'][$type]['bonus_arr']['iframe_text']?></label>
                                        <textarea rows="5" class="form-control for-seo" name="data[bonus][iframe]" id="bonus-iframe"> <?=(isset($bonus['iframe']) && $bonus['iframe'] != '') ? $bonus['iframe'] : ''?></textarea>
                                    </div>
                                    <?php } ?>
                                    <?php } ?>
                                </div>
        ...
        <div class="card card-primary card-outline text-sm">
            <div class="card-header">
                <h3 class="card-title">Bộ sưu tập
                    <?=$config['news'][$type]['title_main']?>
                </h3>
                <div class="card-tools">
                    <button type="button" class="btn btn-tool" data-card-widget="collapse"><i class="fas fa-minus"></i></button>
