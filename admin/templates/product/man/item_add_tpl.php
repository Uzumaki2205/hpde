		                            ...
		                            <?php foreach($config['website']['lang'] as $k => $v) { ?>
		                                <div class="tab-pane fade show <?=($k=='vi')?'active':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang">
		                                    ...
		                                    <?php if(isset($config['product'][$type]['specifications']) && $config['product'][$type]['specifications'] == true) { ?>
		                                        <div class="form-group">
		                                            <label for="specifications<?=$k?>">Quy cách (<?=$k?>):</label>
		                                            <textarea class="form-control for-seo <?=(isset($config['product'][$type]['specifications_cke']) && $config['product'][$type]['specifications_cke'] == true)?'form-control-ckeditor':''?>" name="data[specifications<?=$k?>]" id="specifications<?=$k?>" rows="5" placeholder="Quy cách (<?=$k?>)"><?=htmlspecialchars_decode(@$item['specifications'.$k])?></textarea>
		                                        </div>
		                                    <?php } ?> 
		                                    <?php if(isset($config['product'][$type]['highlights']) && $config['product'][$type]['highlights'] == true) { ?>
		                                        <div class="form-group">
		                                            <label for="highlights<?=$k?>">Điểm nổi bật (<?=$k?>):</label>
		                                            <textarea class="form-control for-seo <?=(isset($config['product'][$type]['highlights_cke']) && $config['product'][$type]['highlights_cke'] == true)?'form-control-ckeditor':''?>" name="data[highlights<?=$k?>]" id="highlights<?=$k?>" rows="5" placeholder="Điểm nổi bật (<?=$k?>)"><?=htmlspecialchars_decode(@$item['highlights'.$k])?></textarea>
		                                        </div>
		                                    <?php } ?>
		                                </div>
		                            <?php } ?>
