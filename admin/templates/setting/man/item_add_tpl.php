		...
		<div class="card card-primary card-outline text-sm">
			...
			<div class="card-body">
							...
							<div class="tab-content" id="custom-tabs-three-tabContent-lang">
								<?php foreach($config['website']['lang'] as $k => $v) { ?>
									...
									<div class="form-group col-md-12 col-sm-12">
										<label for="ten<?=$k?>">Tên công ty (<?=$k?>):</label>
										<input type="text" class="form-control for-seo" name="data[ten<?=$k?>]" id="ten<?=$k?>" placeholder="Tiêu đề (<?=$k?>)" value="<?=@$item['ten'.$k]?>" <?=($k=='vi')?'required':''?>>
									</div>
								</div>
									<div class="tab-pane fade show <?=($k=='vi')?'active':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang">
										<div class="form-group col-md-12 col-sm-12">
											<label for="slogan">Slogan (<?=$k?>):</label>
											<input type="text" class="form-control" name="data[slogan<?=$k?>]" id="slogan<?=$k?>" placeholder="Địa chỉ (<?=$k?>)" value="<?=@$item['slogan'.$k]?>" <?=($k=='vi')?'required':''?>>
										</div>
									</div>
									<div class="tab-pane fade show <?=($k=='vi')?'active':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang">
										<div class="form-group col-md-12 col-sm-12">
				...
				<div class="row">
					...
					<?php } ?>
					<?php if(isset($config['setting']['page_product']) && $config['setting']['page_product'] == true) { ?>
						<div class="form-group col-md-4 col-sm-6">
							<label for="page_product">Phân trang Sản phẩm:</label>
							<input type="text" class="form-control" name="data[options][page_product]" id="page_product" placeholder="Phân trang Sản phẩm" value="<?=$options['page_product']?>">
						</div>
					<?php } ?>
